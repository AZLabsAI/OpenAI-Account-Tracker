    ...
    <>
      ...
      <div
        className={`group relative rounded-2xl border p-6 transition-all duration-200 backdrop-blur-sm overflow-hidden ${
          isPinned
            ? "border-violet-500/30 bg-zinc-900/70 hover:border-violet-500/50"
            : isStarred
      ...
      >
        {/* Left accent strip — pinned takes priority */}
        {isPinned && (
          <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b from-violet-400 to-violet-600" />
        )}
        {!isPinned && isStarred && (
          <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b from-amber-400 to-amber-600" />
        )}
        {!isPinned && !isStarred && isInUse && (
